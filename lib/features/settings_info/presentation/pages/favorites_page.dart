...
import 'package:gold_sham/features/price_detail/presentation/pages/price_detail_page.dart';
...
class _FavoritesPageState extends ConsumerState<FavoritesPage> {
  ...
  Widget _buildItemIcon(PriceItem item) {
    if (item.translatedTitle.contains('24')) {
      return CustomIcon.gold24k(size: 32);
    }
    if (item.translatedTitle.contains('21')) {
      return CustomIcon.gold21k(size: 32);
    }
    if (item.translatedTitle.contains('18')) {
      return CustomIcon.gold18k(size: 32);
    }
    if (item.metalType == 'gold' && item.translatedTitle.contains('auto_str_348'.tr())) {
